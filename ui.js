...
function compileCtoASM(code) {
  ...
  function emitStatement(stmt) {
    ...
    // C/C++: unsigned char val = 0x01; uint8_t i = 0;
    // Java: int val = 0x01; byte i = 0;
    // Go: val := 0x01   OR  var val byte = 0x01
    // Python: val = 0x01 (handled by simple assignment below)
    ...
    if (whileLoop) {
      ...
      asm.push(startLbl + ':');
      // Check if Python (colon at end in original)
  ...
  function parseVal(s) {
    s = s.trim();
    // Strip Go type cast: byte(0x01) → 0x01
    let cast = s.match(/^\w+\((.+)\)$/);
...
function compileIfC() {
  ...
  let baseName = (activeTabKey || 'output').replace(/\.(c|cpp|py|java|go)$/, '');
  let asmKey = baseName + '.asm';

  // Save current .c tab content
  if (activeTabKey) {
    let cur = openTabs.find(t => t.key === activeTabKey);
    if (cur) cur.content = cSrc;
  }

  // Open generated .asm in a new tab (or update existing)
